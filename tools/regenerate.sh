...
if command -v cddlconv &> /dev/null; then
  ...
  if [ "$CDDLCONV_VERSION" != "0.1.10" ]; then
    echo "Warning: 'cddlconv' version is $CDDLCONV_VERSION, but 0.1.10 is required." >&2
    echo "You can install the correct version using: cargo install cddlconv@0.1.10" >&2
  fi
else
  echo "Warning: 'cddlconv' is not installed or not in PATH." >&2
  echo "The build step might fail. You can install it using: cargo install cddlconv@0.1.10" >&2
fi
